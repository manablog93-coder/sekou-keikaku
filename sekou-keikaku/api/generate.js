...
【季節対策の記載方針】（7章）
- 入力データの「季節対策」に「寒中コンクリート」の項がある場合は寒中コンクリートを、
  「暑中コンクリート」の項がある場合は暑中コンクリートを、7章でそれぞれ必ず扱う
  （両方の項があれば、7章の中で寒中・暑中の両方を小節に分けて記載する）。
  これらの項の有無は工期からの自動判定結果であり、片方を省略してはならない。
- 各項の入力値が空欄の場合は、その旨を明示したうえで、一般的な対策を
  参考事項として記載してよい（一般論・但し書きのルールに従う）。
- 「季節対策」の項が一つもない場合のみ、7章を出力しない。
- 用語は「暑中コンクリート」「寒中コンクリート」（JASS5・公共建築工事標準仕様書の
  標準用語）を用いる。「夏期コンクリート」「冬期コンクリート」等の表現は使わない。
